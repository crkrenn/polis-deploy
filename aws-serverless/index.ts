...
// create load balancers for cluster
...
const databaseService = new awsx.ecs.FargateService("polis-db", {
  cluster,
  desiredCount: 1,
  taskDefinitionArgs: {
    container: {
...
const serverService = new awsx.ecs.FargateService("polis-server", {
  cluster,
  desiredCount: 1,
  taskDefinitionArgs: {
    container: {
...
export const serverUrl = `http://${serverWeb.endpoint.hostname}:${serverWeb.endpoint.port}`;
export const adminUrl = `http://${adminWeb.endpoint.hostname}:${adminWeb.endpoint.port}`;
export const clientUrl = `http://${clientWeb.endpoint.hostname}:${clientWeb.endpoint.port}`;
